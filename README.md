...
### 4.5 인증 시스템 (Authentication)

- **역할(Role)**: 사용자 회원가입, 로그인, 세션 관리.
- **의도(Purpose)**:
  - **접근 제어**: 인증된 사용자만 대시보드 기능에 접근 가능.
  - **개인화**: 사용자별 프로필 설정 및 환경 커스터마이징 지원.
- **기술 요소**: bcrypt(비밀번호 해싱), JWT 토큰, SQLAlchemy(User 모델).

### 4.6 프로필 설정 (Profile Settings)

- **역할(Role)**: 사용자 닉네임, 분류 설정 등 개인 정보 관리.
- **의도(Purpose)**:
  - **사용자 경험**: 독립된 사이드바 네비게이션과 직관적인 프로필 편집 UI 제공.
  - **ESG 아이덴티티**: 멸종위기종 테마를 적용하여 환경적 메시지 전달.
- **기술 요소**: React State Management, Material Symbols.
...
| 기술                          | 용도                                                                         |
|:----------------------------- |:---------------------------------------------------------------------------- |
| **ChromaDB (Vector DB)**      | PDF에서 추출된 텍스트 임베딩 저장 및 Cosine Similarity 기반 Semantic Search     |
...
│   │   │   │   └── components/    #     KPICards, TrendChart 등
│   │   │   ├── 경쟁사비교/         #   경쟁사 비교 분석 탭
│   │   │   ├── 시뮬레이터/         #   탄소 시장 시뮬레이터 탭
│   │   │   ├── 목표설정/           #   SBTi 목표 관리 탭
│   │   │   ├── 투자계획/           #   녹색 투자 분석 탭
...
│   │   ├── services/               # API 통신 모듈
│   │   │   ├── api.ts             #   일반 API 호출
│   │   │   └── authApi.ts         #   인증 API (login, signup, token 관리)
...
│   │   ├── types/
│   │   │   └── index.ts           #   TypeScript 타입 정의 (ViewType, TabType 등)
│   │   ├── App.tsx                 # 메인 애플리케이션 (전역 상태 & 네비게이션)
│   │   ├── main.tsx                # React 진입점
│   │   └── index.css               # 전역 스타일
│   ├── index.html
...
> [!NOTE]
>
> - `backend/main.py`: PDF 검색, RAG 챗봇, 문서 관리, 인증 API 포함
...
| 엔드포인트      | 메서드 | 설명                                |
|:--------------- |:------ |:--------------------------------- |
| `/auth/signup`  | POST   | 회원가입 (JSON: email, password)   |
| `/auth/login`   | POST   | 로그인 (JSON: email, password)     |

### 9.2 시뮬레이터 API (`/api/v1/sim`)
